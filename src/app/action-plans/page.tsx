                    ...
                    <div className="space-y-1 w-full md:w-48 shrink-0" onClick={e => e.stopPropagation()}>
                      ...
                      {isCalculated ? (
                        ...
                        <input
                          ...
                          value={progressVal}
                          onChange={(e) => {
                            if (canManageActionPlan(plan)) {
                              handleProgressChange(plan.id, Number(e.target.value), plan.status);
                            }
                          }}
                          disabled={!canManageActionPlan(plan)}
                          className={`w-full h-1.5 bg-slate-100 rounded-lg appearance-none ${canManageActionPlan(plan) ? 'cursor-pointer accent-[#C5A85A]' : 'cursor-default opacity-70'}`}
                        />
                      )}
                      ...
                      <div className="flex justify-between items-center pt-0.5">
                        {isCalculated ? (
                          ...
                          canManageActionPlan(plan) ? (
                            ...
                            <span className="text-[9px] text-slate-400 italic">Visualização apenas</span>
                          )
                        )}
                      </div>
                    </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
            ...
            <div className="flex gap-2">
              {canManageActionPlan(selectedActivePlan) && (
                <button
                            ...
                            <table className="min-w-full divide-y divide-slate-200 text-xs">
                              <thead>
                                <tr className="text-[9px] uppercase font-bold text-slate-400 tracking-wider">
                                  ...
                                  {canManageActionPlan(selectedActivePlan) && <th className="py-2 px-3 text-right w-20">Ações</th>}
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-slate-100 text-slate-700 bg-white">
                                {objective.actions && objective.actions.length > 0 ? (
                                    ...
                                    return (
                                      <tr key={action.id} className="hover:bg-slate-50/60 transition-colors">
                                        ...
                                        <td className="py-2.5 px-3">
                                          {(() => {
                                            const canToggle = canManageActionPlan(selectedActivePlan) || (action.responsible_id === currentProfile?.id);
                                            return (
                                              <button
                                                type="button"
                                                onClick={() => {
                                                  if (canToggle) {
                                                    handleToggleActionStatus(objective.id, action.id);
                                                  }
                                                }}
                                                disabled={!canToggle}
                                                ...
                                                } ${!canToggle ? 'cursor-default opacity-75' : 'hover:scale-105 active:scale-95'}`}
                                                title={canToggle ? (action.status === 'OK' ? 'Marcar em Andamento' : 'Marcar Concluído') : `Status: ${action.status}`}
                                              >
                                        ...
                                        {/* Responsável */}
                                        <td className="py-2.5 px-3 text-slate-555">
                                          {actResponsible}
                                        </td>

                                        {/* Prazo */}
                                        <td className="py-2.5 px-3 text-slate-555 font-medium">
                                          ...
                                          <td className="py-2.5 px-3 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                              ...
                                              <button
                                                ...
                                                className="p-1 rounded text-[#e05656] hover:text-rose-600 hover:bg-rose-50 transition-all"
                                                title="Excluir Ação"
                                              >
                                  ...
                                  <tr>
                                    <td colSpan={canManageActionPlan(selectedActivePlan) ? 6 : 5} className="py-4 text-center text-slate-400 text-[11px] bg-white italic">
            ...
            <form onSubmit={handleSaveActionPlan} className="p-6 space-y-4 text-left">
              ...
              <div className="pt-2">
                <AllowedViewersSelector 
                  ...
                  disabled={selectedPlan ? !canManageActionPlan(selectedPlan) : false}
                />
              </div>
